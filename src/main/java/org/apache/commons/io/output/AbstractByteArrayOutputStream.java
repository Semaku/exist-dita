...
import org.apache.commons.io.NewIOUtils;
...
import static org.apache.commons.io.NewIOUtils.EOF;
...
public abstract class AbstractByteArrayOutputStream extends OutputStream {
    ...
    protected void needNewBuffer(final int newcount) {
        if (currentBufferIndex < buffers.size() - 1) {
            ...
            currentBufferIndex++;
            currentBuffer = NewIOUtils.byteArray(newBufferSize);
    ...
    protected byte[] toByteArrayImpl() {
        int remaining = count;
        if (remaining == 0) {
            return NewIOUtils.EMPTY_BYTE_ARRAY;
        }
        final byte[] newbuf = NewIOUtils.byteArray(remaining);
        int pos = 0;
        for (final byte[] buf : buffers) {
            final int c = Math.min(buf.length, remaining);
